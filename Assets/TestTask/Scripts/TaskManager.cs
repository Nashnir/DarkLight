...
public class TaskManager : MonoSingletion<TaskManager> 
{
    ...
    /// 当前任务列表
    /// </summary>
    private Dictionary<string, Task> currentTaskDic;
    ...
    void Start()
    {
        ...
        currentTaskDic = new Dictionary<string, Task>(DataManager.myTaskDic);
    ...
    void OnApplicationQuit()
    {
        ...
        string path = Application.dataPath + @"/Resources/TXT/MyTask.txt";
        FileInfo info = new FileInfo(path);
        StreamWriter sw = info.CreateText();
        string json = JsonConvert.SerializeObject(currentTaskDic);
        sw.Write(json);
        sw.Close();
        sw.Dispose();
    ...
    public void AcceptTask(string taskID)
    {
        if (currentTaskDic.ContainsKey(taskID))
        {
            Task t = TaskManager.Instance.GetTaskInfoByID(taskID);
